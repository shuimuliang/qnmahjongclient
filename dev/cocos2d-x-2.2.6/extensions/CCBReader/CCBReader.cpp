    ...
    if (memberVarAssignmentType != kCCBTargetTypeNone)
    {
        ...
        else
        {
            if(memberVarAssignmentType == kCCBTargetTypeDocumentRoot) {
                ...
                mOwnerOutletTypes.push_back(className);
            }
        }
    }
...
CCArray* CCBReader::getOwnerOutletTypes() {
    CCArray* pRet = CCArray::createWithCapacity(mOwnerOutletTypes.size());
    std::vector<std::string>::iterator it = mOwnerOutletTypes.begin();
    for (; it != mOwnerOutletTypes.end(); ++it)
    {
        pRet->addObject(CCString::create(*it));
    }
    return pRet;
...
void CCBReader::addOwnerOutletType(std::string type)
{
    mOwnerOutletTypes.push_back(type);
