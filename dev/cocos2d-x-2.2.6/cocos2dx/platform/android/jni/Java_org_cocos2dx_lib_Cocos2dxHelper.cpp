...
void setStringForKeyJNI(const char* pKey, const char* value)
{
    ...
    if (JniHelper::getStaticMethodInfo(t, CLASS_NAME, "setStringForKey", "(Ljava/lang/String;[B)V")) {
        
        int strLen = strlen(value);
        
        jbyteArray byteArray = t.env->NewByteArray(strLen);
        t.env->SetByteArrayRegion(byteArray, 0, strLen, reinterpret_cast<const jbyte*>(value));
        
        
        jstring stringArg1 = t.env->NewStringUTF(pKey);
        t.env->CallStaticVoidMethod(t.classID, t.methodID, stringArg1, byteArray);
